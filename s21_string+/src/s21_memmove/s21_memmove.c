...
static int __is_destination_overlapping_from_left(
    ...
    s21_size_t size) {
        return (0 < destination - source) &&  (destination - source < (signed)size);
}
...
void *s21_memmove(void *destination, const void *source, s21_size_t size) {
    ...
    if (__is_destination_overlapping_from_left(destination_uchar, source_uchar, size))
        return __s21_memcpy_backward_uchar(destination_uchar, source_uchar, size);
