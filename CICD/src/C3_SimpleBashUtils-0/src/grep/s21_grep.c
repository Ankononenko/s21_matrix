...
The resulting executable file must be placed in the directory src/grep/ and
named s21_grep
...
void print_result(Flags const* flags, Data* data) {
  ...
  // If file with patterns doesn't exist, even if "-s" error is thrown and grep
  // doesn't run
  int should_run = TRUE;
  if (flags->f) {
    ...
    for (int p_index = 0; p_index < pattern_lenght; ++p_index) {
      if (data->pattern_array[pattern_index][p_index] ==
        ...
        if (p_index == pattern_lenght - 1 && matches >= pattern_lenght) {
          strcpy(data->inverted_matched_parts_array[inverted_index],
                 temp_array);
          ++inverted_index;
        }
      } else {
...
int parse_line(FILE* file, Data* data) {
  ...
  // Copy regular line to copy. Copy is going to be compared as it changes when
  // flags are applied Regular array gets printed as it is the condition
...
int check_if_files_exist(Flags const* flags, const int filename_index,
  ...
  if (flags->f) {
    ...
    if ((file = fopen(data->all_filenames_array[filename_index], "r")) ==
        NULL) {
      files_exist = FALSE;
    } else {
      fclose(file);
...
void parse_flags(char* argv[], Flags* flags, Data* data, int* counter_for_flags,
  ...
  while (argv[*element_index][letter_index] == '-') {
    ++letter_index;
    // Here I iterate over letters of the flags when they are written in one
    // word
    ...
    ++*element_index;
    // The pattern is being copied to the array of patterns in case the previous
    // flag was -e
...
void parse_filenames(Data* data, char* argv[], int* element_index) {
  while (argv[*element_index]) {
    strcpy(data->all_filenames_array[data->number_of_files],
           argv[*element_index]);
...
void initialize_data(Data* data) {
  ...
  memset(data->line_array, 0, MAX_LENGHT_OF_LINE * sizeof(char));
  memset(data->line_array_copy, 0, MAX_LENGHT_OF_LINE * sizeof(char));
  memset(data->inverted_matched_parts_array, 0,
         sizeof data->inverted_matched_parts_array);
