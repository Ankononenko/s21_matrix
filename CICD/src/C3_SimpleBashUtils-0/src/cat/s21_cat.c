...
- Support of all flags (including GNU versions):
    ...
    2   -e implies -v (GNU only: -E the same, but without implying -v) but also
display end-of-line characters as $i
    3	-n (GNU: --number)	number all output lines
    4	-s (GNU: --squeeze-blank)	squeeze multiple adjacent blank lines
    5	-t implies -v (GNU: -T the same, but without implying -v) but also
display tabs as ^I
- The source, header, and build files must be placed in the src/cat/ directory
- The resulting executable file must be placed in the directory src/cat/
        and named s21_cat
*/
  ...
  for (int index_for_files = 0; index_for_files < data->number_of_files;
    ...
    if (check_if_files_exist(index_for_files, data)) {
      ...
      while (current_character != EOF) {
        handle_flags(&current_character, &next_character, &is_previous_newline,
                     &ordinal, data, flags, file);
      }
      fclose(file);
    } else {
...
void handle_flags(int* current_character, int* next_character,
                  int* is_previous_newline, int* ordinal, Data const* data,
                  Flags const* flags, FILE* file) {
  if (flags->b && *is_previous_newline) {
    handle_b(*current_character, *is_previous_newline, data, ordinal);
  }
  if (flags->s && is_newline(*current_character, data) &&
      *is_previous_newline) {
    handle_s(*current_character, next_character, *is_previous_newline, data,
             file);
  }
  ...
  // Also these conditions are used to not print regular tabs when -t or -T
  // flags are active
...
int is_unprintable(const int current_character, Data const* data) {
  ...
  if (((current_character >= 0 && current_character < 31) &&
       ...
       !is_tabulator(current_character, data)) ||
...
void handle_v(int* current_character, int* next_character, FILE* file,
              Data const* data) {
...
void handle_t(int* current_character, int* next_character, FILE* file,
              Data const* data) {
  while (is_tabulator(*current_character, data) ||
         is_tabulator(*next_character, data)) {
    printf("^I");
    // Used to work around the case when the current char is tab and next is
    // newline If there is no this condtition the newline will get printed and
    // shifted (other conditions wouldn't get checked) For example -e flag
...
void handle_s(const int current_character, int* next_character,
  ...
  while (is_newline(current_character, data) && is_previous_newline &&
...
void handle_b(const int current_character, const int is_previous_newline,
...
void pass_flags_to_structure(Flags* flags, Data const* data) {
  ...
  // strcmp returns 0 if the values are equal, so I use !(not) to invert the
  // value of zero to true
...
int check_if_flags_are_valid(const int counter_for_flags, Data const* data) {
  ...
  while (index_all_flags != counter_for_flags) {
    for (int index_possible_flags = 0;
      ...
      if (!strcmp(data->all_flags_array[index_all_flags],
